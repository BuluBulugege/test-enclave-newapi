...
EXPECTED_MRENCLAVE = os.getenv(
    "EXPECTED_MRENCLAVE",
    "38cc4a089871e1168d7befa0cae3279ee514f2512e70db78c1b890827b30ec5a",
).lower()
...
def chat(req: ChatReq):
    ...
    trace = [
        ...
        {"step": 3, "frm": "new-api 控制面", "to": "SGX Enclave", "secure": False,
         "title": "回传路由决定 + 上游 key（new-api 看不到 prompt）",
         "data": {"channel": "OpenAI 官方", "is_official": True, "upstream_key": "sk-****(库中取)"},
         "note": "new-api 只决定用哪个渠道、给出上游 key；它从头到尾接触不到 prompt。"},
        {"step": 4, "frm": "SGX Enclave", "to": "OpenAI 官方", "secure": True,
         "title": "enclave 用编译写死的官方 URL + 严格 TLS 直连",
         "data": {"url": "https://api.openai.com/v1/chat/completions",
                  "model": req.model, "messages": body["messages"]},
         "note": "目的地 api.openai.com 编译进 enclave 并被 MRENCLAVE 度量，运营方改不了。"},
        {"step": 5, "frm": "OpenAI 官方", "to": "你 (客户端)", "secure": True,
         "title": f"响应经加密内存流回（HTTP {status or '—'}）",
         "data": upstream if upstream is not None else {"error": err},
         "note": ("403 = 香港 IP 被 OpenAI 地域封锁，恰恰证明请求到达了真官方 OpenAI。"
                  if status == 403 else "响应只在加密内存里穿过，不落盘、不入库、不进日志。")},
    ...
    return {"status": status, "error": err, "trace": trace}
